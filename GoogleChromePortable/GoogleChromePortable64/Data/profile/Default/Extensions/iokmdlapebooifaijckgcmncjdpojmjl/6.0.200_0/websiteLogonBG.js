...
var websiteLogonBG = {
    ...
    connectCS: function(port) {
        ...
        this.sendCS(port.tab.id, { cmd: "document_connected", docnum: count, taburl: port.tab.url, tabid: port.tab.id }, count);
    ...
    receiveCS: function(tab, msg) {
        ...
        switch (msg.cmd) {
            ...
            case "openhelpfile":
                ...
                break;
            case "setsyncid":
                this.setSyncID(tab.url, msg.userid, msg.password);
                break;
            ...
            case "frame_connected":
                ...
                if (this._tabsRegInfo[tab.id] && this._tabsRegInfo[tab.id].url && this._tabsRegInfo[tab.id].password) {
                    if (this.isWLOEnabled(tab.url)) {
                        this.sendCS(tab.id, { cmd: "showregister", url: tab.url, otpsupport: this.isOTPSupport(this._tabsRegInfo[tab.id].url), sensorconnected: this.isSensorConnected() }, msg.docnum);
    ...
    onPageLoad: function(msg, tab) {
        ...
        try {
            ...
            if (this._tabsRegInfo[tab.id] && this._tabsRegInfo[tab.id].url) {
                if (msg.url == tab.url && !this._tabsCS[tab.id].frameload) {
    ...
    onFormSubmit: function(msg, tab) {
        ...
        if (this.isWLOEnabled(tab.url)) {
            ...
            if (!this._tabsCS[tab.id].frameload) {
                ...
                this.abortTransaction(tab.id);
                this.sendCS(tab.id, { cmd: "showregister", url: tab.url, otpsupport: this.isOTPSupport(tab.url), sensorconnected: this.isSensorConnected() }, this._tabsCS[tab.id].framedocnum);
                this.sendCS(tab.id, { cmd: "showframe" }, this._tabsCS[tab.id].maindocnum);
            }
        } else {
            this._tabsRegInfo[tab.id] = null;
    ...
    isSensorConnected: function () {
        console.log('<isSensorConnected> entering...');
        try {
            var embed = document.getElementById("embed_npwlo");
            if (embed) {
                var res = embed.IsSensorConnected();
                console.log('<isSensorConnected> result: ' + res);
                return res;
            } else {
                console.log("<isSensorConnected> embed is null");
            }

            return false;
        } catch (e) {
            console.error("<isSensorConnected> Exception:" + e);
        }
    },
