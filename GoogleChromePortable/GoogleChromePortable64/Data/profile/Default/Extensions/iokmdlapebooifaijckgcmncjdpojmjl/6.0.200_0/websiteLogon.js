...
var websiteLogon = {
            ...
            case "document_connected":
                {
                    ...
                    if (this._configHelper && this._configHelper.isSpecialSite() && this._configHelper.addLogonListener(this.specialSiteRegister)) {
    ...
    fillPasswordProfile: function(aFormId, aUserId, aPassword, iSubmit) {
        try {
            ...
            if (websiteLogon._configHelper && websiteLogon._configHelper.isSpecialSite()) {
                websiteLogon._configHelper.logon(aUserId, aPassword);
                return;
            }
    ...
    submitForm: function(aForm) {
        ...
        if (this.submitSpecialWebsites()) {
            isSubmited = true;
        }

        if (!isSubmited) {
            var inputElement = aForm.getElementsByTagName("input");
            for (var j = 0; j < inputElement.length; j++) {
                if (inputElement[j].type == "image") {
                    inputElement[j].click();
                    isSubmited = true;
                    break;
                }
            }
        ...
        if (!isSubmited) {
            var linkelement = aForm.getElementsByTagName("a");

            for (var k = 0; k < linkelement.length; k++) {
                if (linkelement[k].id.indexOf("SignIn") > -1 || linkelement[k].id.indexOf("login") > -1 || linkelement[k].id.indexOf("btnLogin") > -1) {
        ...
        if (!isSubmited) {
            ...
            aForm.dispatchEvent(ev);
        }
    },

    submitSpecialWebsites: function() {
        var website = document.location.href;
        if (website.indexOf("https://www.macys.com/signin") > -1) {
            this.clickElement("name", "SIGNINSIGNIN_BUTTON");
            return true;
        }

        if (website.indexOf("http://www.usatoday.com/community/profile.htm") > -1) {
            this.clickElement("id", "buttonLogIn");
            return true;
        }
    ...
    clickElement: function(condition, elementName) {
        var elemSubmitBtn = null;
        if (condition == "name") {
            // attach the click event process
            //var elemSubmitBtn = document.getElementsByName("login");
            var elemSubmitBtns = document.getElementsByName(elementName);
            if (typeof (elemSubmitBtns) != "undefined" && elemSubmitBtns) {
                elemSubmitBtn = elemSubmitBtns[0];
            }

        }
        else if (condition == "id") {
            // search by id
            elemSubmitBtn = document.getElementById(elementName);
        }
        ...
        try {
            ...
            for (var i = 0; i < document.forms.length; i++) {
                if (this.isOnePasswordForm(document.forms[i])) {
                    ...
                    document.forms[i].addEventListener("click", websiteLogon.onFormClick, false);
                }
            }
    ...
    onFormClick: function(event) {
        ...
        try {
            console.log("<onFormClick> Entering _isSubmitByLogon:" + websiteLogon._isSubmitByLogon);
            if (!websiteLogon._isSubmitByLogon && websiteLogon.isSubmitButton(event)) {
                console.log("<onFormClick> submit button clicked");
                websiteLogon.formRegister(event);
            }
        } catch (e) {
    ...
    specialSiteRegister: function(username, password) {
        console.log("<specialSiteRegister> sepecial website register.");

        if (websiteLogon._isSubmitByLogon) {
            console.log("<specialSiteRegister> plugin logon, should not be registered.");
            return;
        }
        
        //If website is a AJAX website, message "registersucceed" should not be send.
        if (!websiteLogon._configHelper || !websiteLogon._configHelper.isSpecialSite() || websiteLogon._configHelper.getIsRefreshOnLoginFailed()) {
            websiteLogon.sendBG({ cmd: "registersucceed" });
        }

        if (websiteLogon.isWLOEnabled(websiteLogon._currentTabUrl) && !websiteLogon.showDiscoveryDlg(websiteLogon._currentTabUrl, '', username, password)) {
            websiteLogon.sendBG({ cmd: "formsubmit", url: websiteLogon._currentTabUrl, formid: '', userid: username, password: password });
        }
    },
